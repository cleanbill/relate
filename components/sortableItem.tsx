...
const SortableItem = (props: { id: any, value: string, handle: boolean, onReturn: Function, onChange: Function, delete: Function }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <div className="grid grid-cols-[0fr,10fr,1fr]">
      ...
      <input ref={shact} name={field} title={'field ' + props.id} id={'input-' + props.id} className='w-full h-7' onKeyUp={keyup}
        ...
        type='text' ></input>
      <button onClick={e => props.delete(props.id)} className="ml-3 justify-self-start text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm w-6 h-6 dark:bg-red-600 dark:hover:bg-red-700 focus:outline-none dark:focus:ring-red-800 ">X</button>
    </div>
  );
};
